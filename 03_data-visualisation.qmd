...
1.  Be more comfortable with geographic and non-geographic datasets
2.  Produce static visualisations and maps using ggplot
...
## Introducing Today's Dataset(s)

For today's practical, we will be using a couple of different datasets.

Firstly, we will be using data from the latest census - available from NOMIS (https://www.nomisweb.co.uk/sources/census_2021_bulk). In particular, we will be looking at one specific census table; 'Method of Travel to Work', which describes the main method of transport people use to travel to work. For this example, we will be using the table of data that is available for Lower Super Output Areas (LSOAs).
...
My preferred approach to tidying up data or 'data wrangling' is to use the 'tidyverse' library and piping to tidy a data frame in one neat block of code. In the example below I show how you can select two columns, rename them, and then convert to a percentage.
...
Ok, so that's just one example of some steps you might take to tidy up a raw dataset from NOMIS to something a little bit more user friendly. There are lots of additional 'data wrangling' steps you might take as an analyst, some of which we will come onto later on, but for now we just need to apply these techniques to ts061 to get it ready for today's practical.
...
So now we have a nice tidy table, where each variable is now a percentage. The final step is to add a couple of additional geographic variables to the table - local authority district.

The Open Geography Portal (https://geoportal.statistics.gov.uk/datasets/56dfa94d126548b48ceb7bfdd67fb11e/explore) is a great place to find lookup tables for any administrative datasets in the UK. The specific table we have given you provides a lookup between Output Areas (OAs), Lower Super Output Areas (LSOAs), Middle Super Output Areas (MSOAs), Local Enterprise Partnerships (LEPs) and Local Authority Districts (LADs).
...
We're only really interested in aggregating our dataset to Local Authority District, so we can do a number of things to the dataframe to tidy the lookup table:
...
## Static Data Visualisation (Easy)

For most of today's practical, we are going to be using the ggplot package to learn about creating visualisations in R. Ggplot is a really awesome package, it has really excellent documentation and the quality of graphics it can produce is (arguably) second-to-none.
...
First, I want to introduce a couple of key fundamentals when using ggplot. The first relates to specifying the specific dataset that you are using to create a plot. It is very easy to do this:
...
So for example, let's say we are interested in looking at the association between two variables in our dataset:
...
The final fundamental stage is to introduce 'geoms' to our existing plot. Geoms are different types of objects that are used to represent data, including points, bars, lines etc. etc. We will explore lots of these today, but for now, let's just consider plotting a scatter between the two variables in the plot above.
...
Excellent! Your first ggplot visualisation is now ready. It doesn't look the best (right now), but hopefully you have a good understanding of those three fundamental concepts when using ggplot for plotting.

Before we move on to other types of visualisation, let's think about how we can make this plot better, by changing some of the options.

```{r}
## Change some point parameters
ggplot(data = db, aes(x = work_from_home, y = car_driving)) +
...
Using the labs() command enables you to change some of the text that is displayed on the plot. For example, let's change the axis titles to something more useful:
...
The final aesthetic tweak you might make to a plot like this is to change the plot theme. Ggplot has a number of themes that can be selected to change the general appearance of a plot. Here is one example:
...
2.  Explore different themes, and see which one you like most: https://www.datanovia.com/en/blog/ggplot-themes-gallery/.
...
```{r}
## Patrick's attempt
ggplot(data = db, aes(x = foot, y = bicycle)) +
  geom_point(alpha = 0.3, size = 0.35) +
  geom_smooth(method = "lm") +
  #xlim(0, 100) +
  #ylim(0, 100) +
  labs(x = "Population who walk to work (%)", y = "Population who cycle to work (%)",
       caption = "Data: UK Census (2021) - 'Method of travel to work' (ts061)") +
  theme_minimal()
```
...
Now that you have a good understanding of how to construct a plot using ggplot, and how to change some of the parameters to make your plot more visually appealing, we are going to do a quick overview of some simple visualisation techniques and how to build these in ggplot.
...
Alternatively, if you don't like bar-style histograms, you can add the geom_density() command to achieve a similar output:
...
We can also very easily plot a bar chart using ggplot. Let's look at the distribution of LSOAs across LADs. But first, let's filter to those in Liverpool City Region Combined Authority:
...
What if we wanted to look at the distribution of different commuting methods across the LADs?
...
## Static Data Visualisation (Harder)

Ok, so you understand the basics of producing visualisations with ggplot. Now, we are going to work towards building some better visualisations, which are not possible to achieve without learning abit more about reshaping data. If you are familar with pivot tables, its a similar concept!
...
We are first interested in looking at average commuter behaviours between the six Local Authority Districts that make-up Liverpool City Region Combined Authority. To do so, I'm going to introduce two new commands - group_by() and summarise(). As an example, I'll show you how to calculate the average percentage of people who walk to work in each LAD:
...
```{r}
## Plot a bar chart
ggplot(data = walk, aes(x = fct_reorder(LAD22NM, -foot), y = foot)) + ## notice how I've set up the new colum we calculated as the y axis value
...
Well if you look at the code used to produce the bar plot seen above, you'll notice you can only put one command for x and y in the aes() parameter. Thus, we need to reshape our data from wide to long, so that all the %s are within one neat column.
...
There are a few things you can do to change the label used to represent the different colours, firstly you can set a new label using the labs() command:
...
2. Have a go at changing the colour palette used on the plot above, using the scale_fill_brewer() command. Help at: http://www.sthda.com/english/wiki/ggplot2-colors-how-to-change-colors-automatically-and-manually. 
3. See if you can figure out how to facet plot on LAD22NM, instead of applying different colours. Help at: https://www.datacamp.com/tutorial/facets-ggplot-r. 
...
Finally, before we move on to talk about interactive visualisations, I want to do a quick overview of how you can use R to make maps. There is a whole host of GIS functionality within the R ecosystem (see links below), but it also works really well as as a cartographic tool.
...
head(lsoa)
```
The 'geom' column is the most important here - this is what stores the spatial information needed to produce maps. Let's tidy up the polygons before we merge with our other dataset:
...
ggplot(data = lsoa) +
  ...
  labs(fill = "Rail Commuters (%)", caption = "Data: UK Census (2021) - 'Method of travel to work' (ts061)") +
  theme_minimal()
```
...
If you want turn any of the plots we've made today into something interactive, this is really easy! All you need to do is use ggplotly() function from the 'plotly' package.
...
3. (optional) Download another spatial dataset, and have a good at trying to produce a map with it. 
...
A really cool resource is this - Top 50 ggplot2 visualisation (http://r-statistics.co/Top50-Ggplot2-Visualizations-MasterList-R-Code.html) - which has lots of examples of different visualisation techniques that ggplot can be used for.

This book is really awesome too - R for Data Science (https://r4ds.hadley.nz/) - which was written by Hadley Wickham, who introduced ggplot and the tidyverse to the R world.

If interested in using R as a GIS, this free online book is excellent - https://r.geocompx.org/. 
