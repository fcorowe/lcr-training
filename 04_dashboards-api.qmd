...
![Blank Dashboard Template](figs/04/dashboard-raw.png) Now save it **inside your new folder** as something you can remember - e.g., dashboard.Rmd. It is vital that this new .Rmd is saved within your new folder, which should now look like this:
...
![YAML](figs/04/yaml.png) These parameters are doing the following:
...
2. See if you can set up the dashboard to be structured as 1 large pane on the left, and three on the right.
2. (*optional*) Change the section headers for each pane to list some potential visualisations you might put in each, based on yesterday's class.
...
When building a dashboard, a key component will be adding in a number of visualisations to enhance the information being conveyed by the dashboard.

However, the visualisations do need to be produced first, before they can be displayed on the dashboard itself. This is where you can use code chunks within the .Rmd file to re-run some of the analyses we did yesterday to produce visuals for the dashboard.

For example, we can scrape data from the NOMIS API as we did yesterday:

```{r}
## Download the data from NOMIS
db_v2 <- nomis_get_data(id = "NM_2078_1", time = "latest", geography = c("TYPE151"), measures = "20301")
```
...
```{r}
## Clean up the dataset, ready for visualisation 
db_clean <- db_v2 %>%
  select(GEOGRAPHY_CODE, C2021_TTWMETH_12_NAME, OBS_VALUE) %>%
  pivot_wider(names_from = C2021_TTWMETH_12_NAME, values_from = OBS_VALUE) %>%
  setNames(c("LSOA21CD", "total", "work_from_home", "underground_metro", "train", "bus_minibus_coach", 
